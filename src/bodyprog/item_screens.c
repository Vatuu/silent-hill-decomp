...
void GameState_ItemScreens_Update()
{
    ...
    case 16:
        ...
        switch (g_GameWork.gameStateStep_598[1])
        {
        case 12: // "Can't use here" message. (Triggers when using special items in places where they trigger nothing)
            g_GameWork.gameStateStep_598[1] = 1;
            g_GameWork.gameStateStep_598[2] = 0;
			...
			break;
        case 16: // "Too dark too look at the item" message. (Triggers in circumstance like map in otherworld with the flashlight off)
            g_GameWork.gameStateStep_598[1] = 1;
            g_GameWork.gameStateStep_598[2] = 0;
    ...
    case 20:
        ...
        break;
    case 21: // Here the result screen triggers.
...
/** This function draws the two texts that indicate
* that the player can't open the map in the inventory. */
// TODO: RODATA Migration
#ifdef NON_MATCHING
void Gfx_Inventory_UnavailableMapText(s32 stringIdx)
{
    ...
    Gfx_StringSetPosition(0x1E, 0xE8);
    Gfx_StringSetColor(7);
...
/** Name could be inacurate.
* Breaking this function call makes item no longer rotate
* and trying to scroll through the items will softlock the game,
* if the player breaks the function before entering the
* inventory all items will be at the center of the screen
* and the equiped item will appears with a base pose and will keep still.
* If the player doesn't try to scroll through items they
...
/** Used for appearing and disappearing object from the inventory scroll.
*
* Used in: `Inventory_Logic`
*
* In case of removing it, it may cause that items won't appear when scrolling.
* It seems to be conditional for the side that the player is scrolling so
* removing it on some parts of the code will make it not cause any issue when
* scrolling at one side, but will when scrolling at other.
*/
