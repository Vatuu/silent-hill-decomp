...
void vcSetRefPosAndCamPosAngByPad(VECTOR3* ref_pos, s_SysWork* sys_p) // 0x800406D4
{
    ...
    if (!(g_ControllerPtr->btnsHeld_C & ControllerFlag_Circle))
    {
        if (g_ControllerPtr->btnsHeld_C & ControllerFlag_LStickDown)
        {
        ...
        if (g_ControllerPtr->btnsHeld_C & ControllerFlag_LStickRight)
        {
            cam_ang.vy = cam_ang.vy + (g_VBlanks * 11);
        }

        if (g_ControllerPtr->btnsHeld_C & ControllerFlag_LStickLeft)
        {
            cam_ang.vy = cam_ang.vy - (g_VBlanks * 11);
        }

        if (g_ControllerPtr->btnsHeld_C & (ControllerFlag_Triangle | ControllerFlag_Cross))
        {
            var0 = 0;
            if (g_ControllerPtr->btnsHeld_C & ControllerFlag_Triangle)
            {
                var0 = 0x19;
            }
            if (g_ControllerPtr->btnsHeld_C & ControllerFlag_Cross)
            {
                var0 = -0x1A;
            }
    ...
    else
    {
        ...
        if (g_ControllerPtr->btnsHeld_C & (ControllerFlag_LStickRight | ControllerFlag_LStickLeft))
        {
            var0 = 0;
            if (g_ControllerPtr->btnsHeld_C & ControllerFlag_LStickRight)
            {
