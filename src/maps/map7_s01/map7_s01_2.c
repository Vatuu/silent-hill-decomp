...
void func_800DB60C(void) // 0x800DB60C
{
    s32       temp_v1_4;
    s32       i;
    s32       k;
        ...
        case 8:
            ...
            for (i = 0; i < 3; i++)
            {
                for (j = 0; j < 3; j++)
                {
                    for (k = 0; k < 3; k++)
                    {
                        if (D_800E1694 & (1 << ((i * 9 + k * 3) + j)))
                        {
                            setRGBC0(sprt, 0x80, 0x80, 0x80, 0x64);

                            x = D_800E1578[i][j][k][0];
                            y = D_800E1578[i][j][k][1];
    ...
    switch (g_SysWork.sysStateStep_C[0])
    {
        ...
        case 4:
            ...
            D_800E2CA8 += (g_Controller0->sticks_24.sticks_0.leftX << 0xD) / 75;
            D_800E2CA8  = CLAMP_RANGE(D_800E2CA8, -0x73000, 0x73000);

            D_800E2CAC += (g_Controller0->sticks_24.sticks_0.leftY << 0xD) / 75;
            D_800E2CAC  = CLAMP_RANGE(D_800E2CAC, -0x69000, 0x69000);

            Game_TimerUpdate();

            cursorX = FP_FROM(D_800E2CA8, Q12_SHIFT) + 8;
            Gfx_CursorDraw(cursorX, FP_FROM(D_800E2CAC, Q12_SHIFT) + 8, 8, 8, 0, 0x40, 0x20, 0x20, 0x80, 0xC0U, 0, 0xC);
            ...
            if (g_Controller0->btnsClicked_10 & g_GameWorkPtr->config_0.controllerConfig_0.enter_0)
            {
                for (i = 0; i < 3; i++)
                {
                    for (j = 0; j < 3; j++)
                    {
                        for (k = 0; k < 3; k++)
                        {
                            if ((D_800E1578[i][j][k][0] - 0xA9) > FP_FROM(D_800E2CA8, Q12_SHIFT) ||
                                (D_800E1578[i][j][k][0] - 0x97) < FP_FROM(D_800E2CA8, Q12_SHIFT))
                            {
                                continue;
                            }

                            if ((D_800E1578[i][j][k][1] - 0x81) > FP_FROM(D_800E2CAC, Q12_SHIFT) ||
                                (D_800E1578[i][j][k][1] - 0x6F) < FP_FROM(D_800E2CAC, Q12_SHIFT))
                            {
                                continue;
                            }

                            if ((SQUARE(D_800E1578[i][j][k][0] - 0xA0 - FP_FROM(D_800E2CA8, Q12_SHIFT)) +
                                 SQUARE(D_800E1578[i][j][k][1] - 0x78 - FP_FROM(D_800E2CAC, Q12_SHIFT))) > 0x51)
                            {
                                continue;
                            }
                            ...
                            if (!(D_800E1694 & temp_v1_4))
                            {
                                D_800E1694 += temp_v1_4;
                            }
                            else
                            {
                                D_800E1694 -= temp_v1_4;
                            ...
                            if ((D_800E1694 == D_800E1574 && Savegame_EventFlagGet(EventFlag_493)) ||
                                (D_800E1694 == D_800E1570 && Savegame_EventFlagGet(EventFlag_495)))
                            {
                                SysWork_StateStepSet(0, 5);
                            }
                        }
