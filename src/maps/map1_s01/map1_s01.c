...
#include "maps/shared/Ai_LarvalStalker_ControlUpdate.h" // 0x800CE760
...
#include "maps/shared/sharedFunc_800D1DBC_1_s00.h" // 0x800D13B4
...
#include "maps/shared/Ai_Stalker_ControlUpdate.h" // 0x800D272C

void Ai_Stalker_Control_2(s_SubCharacter* chara) {} // 0x800D2ADC

#include "maps/shared/Ai_Stalker_Control_3.h" // 0x800D2AE4

#include "maps/shared/Ai_Stalker_Control_4.h" // 0x800D36B4

#include "maps/shared/Ai_Stalker_Control_5.h" // 0x800D3E20

#include "maps/shared/Ai_Stalker_Control_6.h" // 0x800D4230

#include "maps/shared/Ai_Stalker_Control_7.h" // 0x800D48D0

#include "maps/shared/Ai_Stalker_Control_8.h" // 0x800D49EC

#include "maps/shared/Ai_Stalker_Control_9.h" // 0x800D4D54

#include "maps/shared/Ai_Stalker_Control_10.h" // 0x800D5158

#include "maps/shared/Ai_Stalker_Control_11.h" // 0x800D52D4

#include "maps/shared/Ai_Stalker_Control_12.h" // 0x800D52DC

#include "maps/shared/Ai_Stalker_Control_13.h" // 0x800D5494

#include "maps/shared/sharedFunc_800D67FC_0_s00.h" // 0x800D5584

#include "maps/shared/sharedFunc_800D6970_0_s00.h" // 0x800D56F8

#include "maps/shared/sharedFunc_800D70C4_0_s00.h" // 0x800D5E4C
...
void func_800D9514(void) // 0x800D9514
{
    ...
    if (PLAYER_IN_MAP_CHUNK(vx, 1, -2, -1, -2) && PLAYER_IN_MAP_CHUNK(vz, 1, 3, -1, 3))
    {
        if (Savegame_EventFlagGet(EventFlag_74))
        {
            ...
            {
                g_WorldGfx_ObjectAdd(&g_WorldObject6, &D_800DF080, &(SVECTOR3){ 0, 0, 0 });
            } 
            g_WorldGfx_ObjectAdd(&g_WorldObject5, &D_800DF080, &(SVECTOR3){ 0, 0, 0 });
        }
        else
        {
            g_WorldGfx_ObjectAdd(&g_WorldObject4, &D_800DF080, &(SVECTOR3){ 0, 0, 0 });
        }
    }
    ...
    if (PLAYER_IN_MAP_CHUNK(vx, 1, -3, -1, -3) && PLAYER_IN_MAP_CHUNK(vz, 1, 3, -1, 3))
    {
        ...
        {
            g_WorldGfx_ObjectAdd(&g_WorldObject7.object_0, &g_WorldObject7.position_1C, &(SVECTOR3){ 0, 0, 0 });
        }       
    }
    ...
    if (PLAYER_IN_MAP_CHUNK(vx, 1, -3, -1, -3) && PLAYER_IN_MAP_CHUNK(vz, 0, 0, -1, 1))
    {
        ...
        {
            g_WorldGfx_ObjectAdd(&g_WorldObject8.object_0, &g_WorldObject8.position_1C, &(SVECTOR3){ 0, 0, 0 });
        }
        else
        {
            g_WorldGfx_ObjectAdd(&g_WorldObject9.object_0, &g_WorldObject9.position_1C, &(SVECTOR3){ 0, 0, 0 });
        }
