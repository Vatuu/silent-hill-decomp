...
void Map_WorldObjectsInit(void)
{
    ...
    Math_Vector3Set(&g_WorldObject_Katana.pose_1C.position_0, Q12(-140.8628f), Q12(-0.5511f), Q12(62.3404f));
    WorldObject_ModelNameSet(&g_WorldObject_Katana.object_0, "KATANA1_");

    WorldObjectPoseInit(&g_WorldObject_Cosmo.pose_1C, -145.75f, -0.92f, 18.3274f, 0.0f, -55.1f, 0.0f);
...
void Map_WorldObjectsUpdate(void)
{
    ...
    if (PLAYER_IN_MAP_CHUNK(vx, 1, -3, -1, -3) && PLAYER_IN_MAP_CHUNK(vz, 0, 0, -1, 1))
    {
        ...
        for (i = 0; i < 4; i++)
        {
            g_WorldGfx_ObjectAdd(&g_WorldObject_Winr[i][idx], &g_WorldObjectPose_Winr, &(SVECTOR3){});
        }
    }
     ...
     if (PLAYER_IN_MAP_CHUNK(vx, 1, -5, -1, -5) && PLAYER_IN_MAP_CHUNK(vz, 0, 0, -1, 1))
     {
        ...
        {
            g_WorldGfx_ObjectAdd(&g_WorldObject_Gas.object_0, &g_WorldObject_Gas.pose_1C.position_0, &g_WorldObject_Gas.pose_1C.rotation_C);
        }
    }
    ...
    if (PLAYER_IN_MAP_CHUNK(vx, 1, -4, -1, -4) && PLAYER_IN_MAP_CHUNK(vz, 0, 0, -1, 1))
    {
        ...
        {
            g_WorldGfx_ObjectAdd(&g_WorldObject_Cosmo.object_0, &g_WorldObject_Cosmo.pose_1C.position_0, &g_WorldObject_Cosmo.pose_1C.rotation_C);
        }
    }
    ...
    if (PLAYER_IN_MAP_CHUNK(vx, 1, -4, -1, -4) && PLAYER_IN_MAP_CHUNK(vz, 1, 2, -1, 2))
    {
        ...
        {
            g_WorldGfx_ObjectAdd(&g_WorldObject_Katana.object_0, &g_WorldObject_Katana.pose_1C.position_0, &(SVECTOR3){});
        }
    }
