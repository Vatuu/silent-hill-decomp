...
void func_800EC42C(void) // 0x800EC42C
{
    ...
    block_18:
        ...
        case 8:
            Vc_SetLookAtMatFromBoneCoord(&g_SysWork.playerBoneCoords[HarryBone_Head], &SVECTOR3_Zero, &QVECTOR3(-196.53f, -14.25f, -9.73f));
...
void Map_WorldObjectsUpdate(void) // 0x800ECC8C
{
    ...
    if (PLAYER_IN_MAP_CHUNK(vx, 1, -2, -1, -2) && PLAYER_IN_MAP_CHUNK(vz, 1, -5, -1, -5))
    {
        ...
        {
            WorldGfx_ObjectAdd(&g_CommonWorldObjects[4], &g_CommonWorldObjectPoses[4].position, &g_CommonWorldObjectPoses[4].rotation);
        }
    }
}
