 ...
 * optionally advancing the state step when endTime is reached.
 *
 * If the timer is below startTime, it is clamped to startTime and no advancement occurs.
 * Otherwise, the timer increments by (g_DeltaTime * rate) each frame.
 * When the timer exceeds endTime:
 *   - If clampToEnd is true, the timer is set to exactly endTime.
 *   - If autoAdvance is true, the cutscene state step is incremented.
 *
 ...
 */
void Event_CutsceneTimerAdvance(q19_12* timer, q19_12 rate, q19_12 startTime, q19_12 endTime, bool clampToEnd, bool autoAdvance);
