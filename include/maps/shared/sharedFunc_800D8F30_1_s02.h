...
#define PLAYER_POS (g_SysWork.playerWork_4C.player_0.position_18)
    ...
    {
        xDelta = (PLAYER_POS.vx - creaper->position_18.vx) >> 6;
        zDelta = (PLAYER_POS.vz - creaper->position_18.vz) >> 6;
        if ((SquareRoot0(SQUARE(xDelta) + SQUARE(zDelta)) << 6) < Q12(0.4f))
        {
        ...
        else
        {
            ...
            {
                moveSpeed   = field_10C;
                mvSpeedTmp0 = creaper->moveSpeed_38 - FP_MULTIPLY_PRECISE(g_DeltaTime0, Q12(6.0f), Q12_SHIFT);
                if (moveSpeed < mvSpeedTmp0)
                {
                    moveSpeed = mvSpeedTmp0;
                }
            }
            else
            {
    ...
    {
        pcPos     = creaper->position_18;
        pcPos.vy += creaper->field_C8.field_8;

        func_8008A0E4(1, 53, creaper, &pcPos, &g_SysWork.playerWork_4C.player_0, creaper->rotation_24.vy,
        ...
        if (!(creaper->properties_E4.splitHead.flags_E8 & (1 << 0)))
        {
            func_8005DC1C(Sfx_Unk1424, &creaper->position_18, Q8(0.5f), 0);
        }

        creaper->properties_E4.splitHead.flags_E8 |= (1 << 0);
        ...
        for (i = 0; i < 6; i++)
        {
            ...
            {
                g_SysWork.npcs_1A0[i].properties_E4.splitHead.flags_E8 |= (1 << 8);
            }
        }
    }
    else if (ANIM_STATUS_IDX_GET(creaper->model_0.anim_4.status_0) == 13)
    {
        ...
        creaper->properties_E4.dummy.properties_E8[8].val16[0] = func_8006FAFC(creaper, Q12(4.8f), PLAYER_POS.vx, PLAYER_POS.vz, FP_ANGLE(360.0f), false);
