...
void Ai_Creaper_Init(s_SubCharacter* creaper)
{
    ...
    do
    {
        if (creaper == &g_SysWork.npcs_1A0[i] || g_SysWork.npcs_1A0[i].model_0.charaId_0 != Chara_Creaper)
        {
            i++;
            continue;
        }
    ...
    ModelAnim_AnimInfoSet(&creaper->model_0.anim_4, CREAPER_ANIM_INFOS);
...
#ifdef MAP5_S00
    creaper->flags_3E |= CharaFlag_Unk9;
#endif
}
