void sharedFunc_800D9528_1_s02(s_SubCharacter* creaper)
{
    ...
    if (creaper->model_0.anim_4.status_0 == ANIM_STATUS(CreaperAnim_7, true))
    {
        ...
        {
            q19_12 unkDistIntermediate = FP_MULTIPLY_PRECISE(CREAPER_ANIM_INFOS[creaper->model_0.anim_4.status_0].duration_8.constant, g_DeltaTime0, Q12_SHIFT) * Q12(0.1f);
            dist                       = unkDistIntermediate / Q12(4.0f);
        }

        creaper->properties_E4.dummy.properties_E8[1].val16[0] = FP_FROM(dist * Math_Sin(creaper->rotation_24.vy + FP_ANGLE(180.0f)), Q12_SHIFT);
        creaper->properties_E4.dummy.properties_E8[1].val16[1] = FP_FROM(dist * Math_Cos(creaper->rotation_24.vy + FP_ANGLE(180.0f)), Q12_SHIFT);
    }
