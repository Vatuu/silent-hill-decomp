void sharedFunc_800D94EC_2_s00(s_SubCharacter* airScreamer)
{
    s32  animStatus;
    s32  switchCond;
    s32  sp10;
    bool cond;
    bool field14C_0;
    bool field14C_1;
    bool field14C_2;
    bool field14C_0_tmp;
    bool field14C_1_tmp;
    bool field14C_2_tmp;
    s32  temp_s4;
    s32  distFieldF8;
    s32  angleDiff;

    animStatus = airScreamer->model_0.anim_4.status_0;
    switchCond = 0;
    sp10       = sharedFunc_800D4A80_0_s01(airScreamer);
    cond       = 0;

    field14C_0_tmp = sharedData_800E21D0_0_s01.field_14C.bits32.field_14C_0;
    field14C_1_tmp = sharedData_800E21D0_0_s01.field_14C.bits32.field_14C_1;
    field14C_2_tmp = sharedData_800E21D0_0_s01.field_14C.bits32.field_14C_2;

    field14C_0 = field14C_0_tmp;
    field14C_1 = field14C_1_tmp;
    field14C_2 = field14C_2_tmp;

    temp_s4 = sharedFunc_800DC438_2_s00(airScreamer);

    switch (airScreamer->model_0.stateStep_3)
    {
        case 64:
            sharedFunc_800DD534_2_s00(airScreamer);
            airScreamer->model_0.stateStep_3 = 1;
            break;

        case 0:
            cond = true;

            airScreamer->properties_E4.unk0.properties_120.val32 = Q12(5.0f);
            sharedFunc_800DEC84_2_s00(airScreamer, Q12(6.0f), airScreamer->rotation_24.vy);
            sharedFunc_800DECA4_2_s00(airScreamer, &airScreamer->properties_E4.unk0.field_F8, Q12(5.0f));

            airScreamer->model_0.stateStep_3 = 1;

        case 1:
            distFieldF8 = Math_Distance2dGet(&airScreamer->position_18, &airScreamer->properties_E4.unk0.field_F8);
            angleDiff   = FP_ANGLE_NORM_S(func_80080478(&airScreamer->position_18, &airScreamer->properties_E4.unk0.field_F8) - airScreamer->rotation_24.vy);

            if (temp_s4 == 2)
            {
                airScreamer->model_0.stateStep_3 = 2;
            }
            else if (temp_s4 == 1 || (sharedFunc_800DC30C_2_s00(airScreamer) && 
                                      cond == true && 
                                      airScreamer->properties_E4.unk0.flags_11C & CharaUnk0Flag_Unk31 && 
                                      (distFieldF8 < Q12(2.0f) || Math_CheckSignedRange(angleDiff, FP_ANGLE(60.0f)))))
            {
                airScreamer->model_0.stateStep_3 = 4;
            }
            else if (sharedFunc_800DC3BC_2_s00(airScreamer) && 
                     cond == true && 
                     distFieldF8 < Q12(4.0f) && 
                     (angleDiff >= FP_ANGLE(-30.0f) && angleDiff <= FP_ANGLE(30.0f)) && 
                     Rng_RandQ12() < Q12(0.7f))
            {
                airScreamer->model_0.stateStep_3 = 2;
            }
            else if (airScreamer->properties_E4.unk0.properties_120.val32 == 0 || distFieldF8 < Q12(3.0f))
            {
                airScreamer->model_0.stateStep_3 = 0;
            }
            break;

        case 2:
            if (animStatus == ANIM_STATUS(17, true))
            {
                switchCond                     = 1;
                airScreamer->model_0.anim_4.status_0 = ANIM_STATUS(16, false);
                airScreamer->model_0.stateStep_3     = 3;
            }
            break;

        case 3:
            switchCond = 1;
            break;

        case 4:
            if (animStatus == ANIM_STATUS(17, true))
            {
                switchCond                     = 2;
                airScreamer->model_0.anim_4.status_0 = ANIM_STATUS(15, false);
                airScreamer->model_0.stateStep_3     = 5;
            }
            break;

        case 5:
            switchCond = 2;
            break;

        case 65:
            sharedFunc_800DDA80_2_s00(airScreamer);
            airScreamer->model_0.stateStep_3 = 1;
            break;

        case 66:
            sharedFunc_800DD588_2_s00(airScreamer);
            airScreamer->model_0.stateStep_3 = 1;
            break;

        case 67:
            sharedFunc_800DD834_2_s00(airScreamer);
            airScreamer->model_0.stateStep_3           = 1;
            airScreamer->properties_E4.unk0.flags_11C |= CharaUnk0Flag_Unk12;
            break;
    }

    sharedFunc_800E021C_2_s00(airScreamer, 0, 1);

    switch (Ai_AirScreamer_DamageTake(airScreamer, Q12(1.0f)))
    {
        case 0:
            switch (switchCond)
            {
                case 0:
                    if (field14C_1 | field14C_0 | field14C_2)
                    {
                        airScreamer->model_0.controlState_2     = 35;
                        airScreamer->model_0.stateStep_3 = 0;
                    }
                    break;

                case 1:
                    if (sp10 == 1)
                    {
                        if (!(field14C_1 | field14C_0 | field14C_2))
                        {
                            airScreamer->model_0.controlState_2     = 4;
                            airScreamer->model_0.stateStep_3 = 0;
                        }
                        else
                        {
                            airScreamer->model_0.controlState_2     = 6;
                            airScreamer->model_0.stateStep_3 = 0;
                        }
                    }
                    break;

                case 2:
                    if (sp10 == 2)
                    {
                        if (!(field14C_1 | field14C_0 | field14C_2))
                        {
                            airScreamer->model_0.controlState_2     = 18;
                            airScreamer->model_0.stateStep_3 = 0;
                        }
                        else
                        {
                            airScreamer->model_0.controlState_2     = 20;
                            airScreamer->model_0.stateStep_3 = 0;
                        }
                    }
                    break;
            }
            break;

        case 1:
        case 2:
            airScreamer->model_0.controlState_2               = 44;
            airScreamer->model_0.stateStep_3           = 0;
            airScreamer->properties_E4.unk0.flags_11C |= CharaUnk0Flag_Unk3;
            break;

        case 3:
        case 4:
            airScreamer->model_0.controlState_2     = 45;
            airScreamer->model_0.stateStep_3 = 0;
            if (airScreamer->health_B0 <= 0)
            {
                airScreamer->properties_E4.unk0.flags_11C |= CharaUnk0Flag_Unk6;
            }
            else
            {
                airScreamer->properties_E4.unk0.flags_11C |= CharaUnk0Flag_Unk3;
            }
            break;
    }
}
