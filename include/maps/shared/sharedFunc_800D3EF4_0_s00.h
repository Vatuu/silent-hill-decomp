void sharedFunc_800D3EF4_0_s00(s_SubCharacter* stalker)
{
        ...
        !func_800700F8(stalker, &g_SysWork.playerWork_4C.player_0))
    {
        ...
        {
            if (ABS(angleToPlayer) > FP_ANGLE(90.0f))
            {
                Chara_MoveSpeedUpdate(stalker, Q12(1.5f));
            }
            else
            {
                Chara_MoveSpeedUpdate3(stalker, FP_ANGLE(180.0f), (FP_ANGLE(90.0f) - ABS(angleToPlayer)) * 4);
            }
        }

        for (i = 3 + ((ABS(angleToPlayer) > FP_ANGLE(90.0f)) ? 1 : 0); i > 0; i--)
        {
            if (((g_DeltaTime0 >> 2) + 1) < ABS(angleToPlayer))
            {
            ...
            if (ABS(angleToPlayer) < ((g_DeltaTime0 >> 2) + 1) || distToPlayer > Q12(2.0f))
            {
                break;
            }
        }
            ...
            (!(stalker->properties_E4.stalker.flags_E8 & StalkerFlag_2) && Rng_GenerateInt(0, 1) != 0))   // 1 in 2 chance.
        {
            if ( distToPlayer < Q12(0.9f) &&
                (distToPlayer > Q12(0.7f) || Rng_GenerateInt(0, 3) == 0)) // 1 in 4 chance.
            {
                if (ABS(angleToPlayer) < FP_ANGLE(90.0f) && g_SysWork.playerWork_4C.player_0.health_B0 > Q12(0.0f))
                {
        ...
        {
            if (ABS(angleToPlayer) < FP_ANGLE(60.0f))
            {
