s32 sharedFunc_800D3508_0_s01(s_SubCharacter* chara, s32* arg1)
{
    ...
    if (ret)
    {
        tmp = ratan2(g_SysWork.player_4C.chara_0.position_18.vx - chara->position_18.vx, g_SysWork.player_4C.chara_0.position_18.vz - chara->position_18.vz);
        angle = tmp;
        temp_lo = g_SysWork.field_2388.field_154.field_0.field_0.s_field_0.field_0;
        if (!(temp_lo & 2))
        {
            // @hack permuter find.
            temp_lo = 0; while (temp_lo){};
            do {
                sharedFunc_800D4A80_0_s01(chara);
            } while (0);
        }

        tmp = FP_MULTIPLY_PRECISE(temp_s2, Math_Sin(angle), Q12_SHIFT);
        temp_hi = tmp;
